...
# function to compose the whole addess of a node for a given split
...
for node in doc.nodes:
    if treebank == 'ITTB':
        ...
        elif node.udeprel == 'nsubj' and node.parent.deprel == 'advcl:abs' and node.parent.feats['VerbForm'] == 'Part' and not node.parent.feats['Gender'] and node.feats['Case'] == node.parent.feats['Case']:
...
for node in doc.nodes:
    ...
    if node.upos == 'AUX':
        if treebank == 'ITTB':
            ...
            if 'gen4' in node.xpos:
                ...
                node.feats['VerbForm'], node.feats['Aspect'], node.feats['Tense'] = 'Inf', 'Imp', 'Pres'
        
        elif treebank == 'LLCT':
            if node.lemma == 'sum':
                node.feats['Voice'] = ''
            if node.form in ['sum', 'es', 'is', 'his', 'et', 'est', 'sumus', 'summus', 'estis', 'sunt', 'sit', 'siit', 'sia', 'simus', 'sint', 'essem', 'esset', 'essed', 'esse', 'essere', 'habemus', 'abemus', 'abetis']:
    ...
    if node.upos in ['VERB', 'AUX']:
        if not node.feats['Tense']:
            if node.xpos and 'tem1' in node.xpos:
        ...
        elif node.feats['Mood'] == 'Ind':
            node.misc['TraditionalMood'] = 'Indicativus'
        elif node.feats['Mood'] == 'Sub':
            node.misc['TraditionalMood'] = 'Subiunctivus'
        elif node.feats['Mood'] == 'Imp':
            node.misc['TraditionalMood'] = 'Imperativus'
        if node.feats['Mood'] in ['Imp', 'Ind', 'Sub']:
            if node.feats['Tense'] == 'Pres':
                node.misc['TraditionalTense'] = 'Praesens'
            elif node.feats['Tense'] == 'Fut':
                node.misc['TraditionalTense'] = 'Futurum'
        ...
        if node.feats['VerbForm'] == 'Inf' and not node.feats['Tense']:
            if 'tem4' in node.xpos:
                node.feats['Tense'] = 'Past'
            elif node.form == 'posse':
                node.feats['Tense'] = 'Pres'

    
    elif node.upos == 'DET':
        if node.lemma in ['qualis', 'quantus', 'quicumque', 'quot'] and (not node.feats['PronType'] or ',' in node.feats['PronType']):
            node.feats['PronType'] = 'Rel'
            if node.lemma in ['quantus', 'quot']:
                node.feats['NumType'] = 'Card'
        elif node.lemma in ['aliqualis', 'aliqui', 'nullus', 'qualiscumque', 'quidam', 'quisque', 'quilibet', 'quotlibet'] and (not node.feats['PronType'] or ',' in node.feats['PronType'] or node.feats['PronType'] == 'Tot'):
    ...
    if treebank == 'ITTB':
        if node.deprel == 'nmod' and not node.feats['Gender'] and node.feats['Case'] == node.parent.feats['Case'] and node.feats['Number'] == node.parent.feats['Number'] and not node.feats['Abbr'] and node.feats['InflClass'] != 'Ind':
            node.feats['Gender'] = node.parent.feats['Gender']


    if node.upos == 'NUM':
        if node.lemma in ['unus', 'duo', 'tres', 'quattuor', 'quatuor', 'quator', 'quinque', 'sex', 'septem', 'octo', 'novem', 'nouem', 'decem', 'undecim', 'duodecim', 'tredecim', 'quattuordecim', 'quindecim', 'sedecim', 'septendecim', 'duodeviginti', 'undeviginti', 'uiginti', 'viginti', 'duodetreginta', 'triginta', 'quadraginta', 'quinquaginta', 'sexaginta', 'septuaginta', 'octoginta', 'nonaginta', 'centum', 'ducenti', 'trecenti', 'quadringenti', 'quingenti', 'sescenti', 'octingenti', 'octogenti', 'mille', 'milia']:
        ...
        if node.lemma == 'sui':
            node.feats['Gender'], node.feats['PronType'], node.feats['Person'], node.feats['Reflex'] = '', 'Prs', '3', 'Yes'
    ...
    if treebank == 'ITTB':
        if node.upos in ['ADJ', 'DET', 'NOUN', 'VERB'] and node.xpos and 'casG' in node.xpos:
                ...
                node.feats['Aspect'], node.feats['InflClass[nominal]'], node.feats['VerbForm'], node.feats['Voice'] = '', '', '', ''
                if node.form[-3:] == 'ius':
    ...
    if treebank == 'ITTB':
        ...
        if node.address() == ad('1957#13'):
            ...
            node.feats['Gender'] = node.parent.feats['Gender']
        elif (node.address() in [ad('2362#13'), ad('7790#14'), ad('1337#31'), ad('8480#36'), ad('10582#9')]) and node.lemma == 'quod':
