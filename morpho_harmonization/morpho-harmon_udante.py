...
for node in doc.nodes:
    ...
    if node.upos in ['AUX', 'VERB']:
        ...
        if node.feats['VerbForm'] in ['Inf', 'Vnoun'] and not node.feats['Tense']:
                if node.feats['Aspect'] in ['Imp', 'Inch']:
                    node.feats['Tense'] = 'Pres'
                elif node.feats['Aspect'] == 'Perf':
                    node.feats['Tense'] = 'Past'
                elif node.feats['Aspect'] == 'Prosp':
    ...
    # verbal features
    if not node.feats['Voice'] and node.form.lower() in ['abstinebam', 'ades', 'adesset', 'adest', 'adirem', 'adsolet', 'advolaverit', 'afferunt', 'affuerit', 'cadat', 'caderet', 'cadet', 'competebat', 'displicet', 'distribuit', 'exarserat', 'exivit', 'exueras', 'infit', 'innuit', 'noluerunt', 'preponderet', 'regnat', 'retulit', 'sufficiunt', 'tendit', 'vacet', 'venerat']:
    ...
    if not node.feats['Case']:
        if node.address() in ['DVE-265#49', 'Egl-26#13', 'Egl-26#34', 'Egl-86#20', 'Epi-60#5', 'Epi-296#16', 'Epi-300#3', 'Mon-286#50', 'Mon-465#9', 'Que-91#7'] or node.form.lower() == 'ego':
            node.feats['Case'] = 'Nom'   
    ...
    if not node.feats['Gender']:
        if node.form in ['destructa', 'peritura', 'alienate', 'edite', 'solite', 'sortite', 'subiecte'] or node.address() in ['Egl-87#22', 'Epi-200#10', 'Mon-216#1', 'Mon-425#36']:
